...
function! s:render(nodes) abort
  ...
  if options.indent_markers
    ...
    for i in range(length_nodes - 1, 0, -1)
      let node = a:nodes[i]
      let node._renderer_devicons_level = len(node.__key) - base
      let node._renderer_devicons_last = get(levels, node._renderer_devicons_level, 0) isnot# 1 ? 1 : 0

      for key in keys(levels)
        if key > node._renderer_devicons_level
          let levels[key] = 0
        endif
      endfor

      let levels[node._renderer_devicons_level] = 1
    endfor

    for i in range(length_nodes)
      let node = a:nodes[i]
      let last_marker = i is# 0 ? [0] : a:nodes[i - 1]._renderer_devicons_marker
      let node._renderer_devicons_marker = [repeat([0], node._renderer_devicons_level)][0]
      let current_length = len(node._renderer_devicons_marker)

      for ii in range(min([len(last_marker), current_length]))
        let node._renderer_devicons_marker[ii] = last_marker[ii]
      endfor

      if node._renderer_devicons_last is# 1 && i isnot# 0
        let node._renderer_devicons_marker[current_length - 1] = 1
      endif
    endfor
  endif
...
function! s:render_node(node, base, options) abort
  ...
  if a:options.indent_markers
    let indent_length = len(a:node._renderer_devicons_marker)

    for i in range(indent_length)
      let indent = a:node._renderer_devicons_marker[i]
